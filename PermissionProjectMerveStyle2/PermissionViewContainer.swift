...
class PermissionViewContainer : UIView {
    ...
    func setupView() {
        ...
        let positiveButton = makeButton(with: "OK!", fontAndSize: FontManager.regular(24).value, type: .filled, colour: colour)
        positiveButton.addTarget(self, action: #selector(permissionClicked), for: .touchUpInside)
        let negativeButton = makeButton(with: "Not Now!", fontAndSize: FontManager.regular(24).value, type: .outlined, colour: colour)
